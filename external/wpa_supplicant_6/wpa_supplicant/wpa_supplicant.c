...
#ifdef WAPI_GET_CERT
static int wapi_keystore_get(const char *key, int length, char *value)
{
    uint8_t bytes[2];
    uint8_t code = 'g';
    int sock;
		...
		#ifdef WAPI_CERT_FROM_KEYSTORE
		{
			...
			if (os_strncmp("keystore://", (char *)user_cert, 11) == 0) {
...
#ifdef WAPI_GET_CERT
                {
                    ...
                    length = wapi_keystore_get(user_cert_tmp, key_len, value);
                }
#else
			...
			if (os_strncmp("keystore://", (char *)as_cert, 11) == 0) {
...
#ifdef WAPI_GET_CERT
                {
                    ...
                    length = wapi_keystore_get(as_cert_tmp, key_len, value);
                }
#else
