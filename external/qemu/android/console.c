...
static const CommandDefRec  sms_commands[] =
{
    ...
    { "send", "send inbound SMS text message to sim1",
    "'sms send <phonenumber> <message>' allows you to simulate a new inbound sms message to sim2\r\n", NULL,
    do_sms_send, NULL },
...
static int
do_accelerometer_update( ControlClient  client, char*  args )
{
#define NUM_SENSOR_ACC_PARAMS 3
    char*   p = args;
    int     top_param = -1;
    double  params[ NUM_SENSOR_ACC_PARAMS ];
    ...
    while (*p) {
        ...
        if (top_param + 1 == NUM_SENSOR_ACC_PARAMS)
            break;
    ...
    /* sanity check */
    if (top_param < NUM_SENSOR_ACC_PARAMS - 1) {
        control_write( client, "KO: not enough arguments: see 'help sensor acc'"
        	" for details\r\n" );
        return -1;
    }

    android_hw_set_acceleration( params[0], params[1], params[2] );

    return 0;
}

static int
do_magnetic_field_sensor_update( ControlClient  client, char*  args )
{
#define NUM_SENSOR_MAG_PARAMS 3
    char*   p = args;
    int     top_param = -1;
    double  params[ NUM_SENSOR_MAG_PARAMS ];
    ...
    while (*p) {
        ...
        params[++top_param] = val;
        if (top_param + 1 == NUM_SENSOR_MAG_PARAMS)
            break;
    ...
    /* sanity check */
    if (top_param < NUM_SENSOR_MAG_PARAMS - 1) {
        control_write( client, "KO: not enough arguments: see 'help sensor mag'"
        	" for details\r\n" );
        return -1;
    }

    android_hw_set_magnetic_field( params[0], params[1], params[2] );

    return 0;
}

static int
do_orientation_sensor_update( ControlClient  client, char*  args )
{
#define NUM_SENSOR_ORI_PARAMS 3
    char*   p = args;
    int     top_param = -1;
    double  params[ NUM_SENSOR_ORI_PARAMS ];
    ...
    while (*p) {
        ...
        params[++top_param] = val;
        if (top_param + 1 == NUM_SENSOR_ORI_PARAMS)
            break;
    ...
    /* sanity check */
    if (top_param < NUM_SENSOR_ORI_PARAMS - 1) {
        control_write( client, "KO: not enough arguments: see 'help sensor ori' for details\r\n" );
        return -1;
    }

    android_hw_set_orientation( params[0], params[1], params[2] );

    return 0;
}

static int
do_temperature_sensor_update( ControlClient  client, char*  args )
{
#define NUM_SENSOR_TMP_PARAMS 1
    char*   p = args;
    int     top_param = -1;
    double  params[ NUM_SENSOR_TMP_PARAMS ];
    ...
    while (*p) {
        ...
        params[++top_param] = val;
        if (top_param + 1 == NUM_SENSOR_TMP_PARAMS)
            break;
    ...
    /* sanity check */
    if (top_param < NUM_SENSOR_TMP_PARAMS - 1) {
        control_write( client, "KO: not enough arguments: see 'help sensor tmp'"
        	" for details\r\n" );
        return -1;
    }

    android_hw_set_temperature( params[0] );
...
{
   { "acc", "change accelerometer values",
   "'sensor acc <x> <y> <z>' allows you to change the values of the 3-axis"
   " accelerometer at runtime\r\n"
   " The parameters are:\r\n\r\n"
   "  <x>    acceleration of X axis, in decimal m/s^2\r\n"
   "  <y>    acceleration of Y axis, in decimal m/s^2\r\n"
   "  <z>    acceleration of Z axis, in decimal m/s^2\r\n"
   "\r\n",
   NULL, do_accelerometer_update, NULL },
   
   { "mag", "change magnetic field sensor values of the 3-axis",
   "'sensor mag <x> <y> <z>' allows you to change the values of at runtime\r\n"
   " The parameters are:\r\n\r\n"
   "  <x>    magnetic field of X axis, in micro-Tesla (uT)\r\n"
   "  <y>    magnetic field of Y axis, in micro-Tesla (uT)\r\n"
   "  <z>    magnetic field of Z axis, in micro-Tesla (uT)\r\n"
   "\r\n",
   NULL, do_magnetic_field_sensor_update, NULL },

   { "ori", "change orientation sensor values of the 3-axis",
   "'sensor ori <azimuth> <pitch> <roll>' allows you to change the values of at"
   " runtime\r\n"
   " The parameters are:\r\n\r\n"
   "  <azimuth>	Azimuth, angle (0 ~ 359) between the X axis of the phone and the"
   " magnetic north direction, in degrees\r\n"
   "            0 = North, 90 = East, 180 = South, 270 = West"
   "  <pitch>	Pitch, rotation around the X axis (-180 ~ 180), in degrees\r\n"
   "  <roll>	Roll, rotation around the Y axis (-90 ~ 90), in degrees\r\n"
   "\r\n",
   NULL, do_orientation_sensor_update, NULL },
   
   { "tmp", "change temperature sensor value",
   "'sensor tmp <celsius>' allows you to change the value of at runtime\r\n"
   " The parameters are:\r\n\r\n"
   "  <celsius>    device temperature, in degrees centigrade (Celsius)\r\n"
   "\r\n",
   NULL, do_temperature_sensor_update, NULL },
...
static const CommandDefRec   main_commands[] =
{
    ...
    { "sensor", "sensor related commands",
      "allows to change various sensor values\r\n", NULL,
