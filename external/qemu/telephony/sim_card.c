...
{
 printf("[Emu] asimcard_check_pin %d\r\n",sim->status);
    if (sim->status != A_SIM_STATUS_PIN   &&
        sim->status != A_SIM_STATUS_READY )
        {printf("[Emu] asimcard_check_pin 1\r\n");
        return 0;}
    printf("[Emu] asimcard_check_pin %s\r\n",sim->pin);
	printf("[Emu] asimcard_check_pin %s\r\n",pin);
    if ( !strcmp( sim->pin, pin ) ) {
        sim->status      = A_SIM_STATUS_READY;
        sim->pin_retries = 3;
        printf("[Emu] asimcard_check_pin 2\r\n");
        return 1;
    }
...
{
    printf("[Emu] asimcard_check_puk1 \r\n");
    if (sim->status != A_SIM_STATUS_PUK)
        return 0;
    printf("[Emu] asimcard_check_puk2 \r\n");
...
asimcard_io( ASimCard  sim, const char*  cmd )
{
	...
	//new get device log and modify below.
	/*
        { "+CRSM=192,12258,0,0,15", "+CRSM: 144,0,566f6963656d61696cffffffffffffffffff07915155125740f9ffffffffffff" },
